...
mcp.registerTool(
  ...
  async ({ cards, rules, format = 'json' }) => {
    ...
    if (format === 'html') {
      ...
      const rawData = Buffer.from(encodeURI(JSON.stringify(deckObj))).toString('base64');
      const html = `<!DOCTYPE html><html><head><script>const rawData = '${rawData}'; const _deck = JSON.parse(decodeURI(atob(rawData))); const cards = Array.isArray(_deck) ? _deck : (_deck.cards ?? []);</script><title>Blank White Cards</title></head><body><p>Blank White Cards deck file. Open in the <a href="https://blankwhite.cards/editor">Deck Editor</a> to view and edit.</p></body></html>`;
